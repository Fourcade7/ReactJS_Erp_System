    ...
    <ListGroup as="ol"  className="rounded overflow-hidden">
      ...
      {products.map((user,index) => (
        <ListGroup.Item
          key={user.id}
          as="li"
          className={`d-flex ${index % 2 === 1 ? "bg-light" : "bg-dark-subtle "}`}
        >
          <div className='d-flex flex-row w-100'>
            <div className='d-flex'>
              <small className={`me-2 m-0 p-0  px-2 rounded mt-0 ${index % 2 === 1 ? "bg-dark-subtle" : "bg-light"}`}>{user.id}</small>
...
function PurchaseTab() {
  return (
    <Tabs
      ...
      //style={{fontSize:"12px"}}
    >
      <Tab eventKey="home" title="Список прихода">
         <PurchaseListGroup></PurchaseListGroup>
      </Tab>
      <Tab eventKey="profile" title="Добавить новый приход">
